...
public readonly partial struct Box<T> : global::System.IEquatable<Box<T>>, global::System.IDisposable
    where T : global::System.IDisposable
{
    ...
    /// <summary>Removes ownership of the Some payload from <paramref name="self"/> and returns it. Sets <paramref name="self"/> to default so subsequent disposal is a no-op. Throws if the active variant is not Some.</summary>
    public static T TakeSome(ref Box<T> self)
    {
        if (self._tag != Tags.Some) ThrowInvalidCase(nameof(Some));
        var value = self._some_value;
        self = default;
        return value;
    }

    /// <summary>If the active variant is Some, removes ownership of its payload, writes it to <paramref name="value"/>, sets <paramref name="self"/> to default, and returns true.</summary>
    public static bool TryTakeSome(ref Box<T> self, out T value)
    {
        if (self._tag == Tags.Some)
        {
            value = self._some_value;
            self = default;
            return true;
        }
